...
pub fn start(ui: &mut UI, key_cache: &KeyCache) -> Result<()> {
    ui.br()?;
    ui.title(format!("{} CLI Setup", PRODUCT_NAME))?;
    ui.para(&format!("Welcome to {} cli setup. Let's get started.", CLI_NAME))?;

    ui.heading(format!("{} Builder Instance", PRODUCT_NAME))?;
    ui.para(&format!("{} packages can be stored in either the public builder instance {} or in \
                      an on-premises builder depot instance. If you do not set a builder URL \
                      now, the `{}` CLI will default to using the public builder instance. \
                      This can be overridden at any time after setup.",
                     PRODUCT_NAME, DEFAULT_BUILDER_URL, CLI_NAME))?;
    ui.br()?;
    if ask_default_builder_instance(ui)? {
        ui.para(&format!("Enter the url of your builder instance. The default is {}. The \
                          configured endpoint can be overridden any time with a `HAB_BLDR_URL` \
                          environment variable or a --url flag on the cli.",
                         DEFAULT_BUILDER_URL))?;
    ...
    ui.para(&format!("Every package in {} belongs to an origin, which indicates the person or \
                      organization responsible for maintaining that package. Each origin also \
                      has a key used to cryptographically sign packages in that origin.",
                     PRODUCT_NAME))?;
    ui.para(&format!("Selecting a default origin tells package building operations such as '{} \
                     ...
                     CLI_NAME))?;
    ui.para(&format!("For more information on origins and how they are used in building \
                      packages, please consult the docs at {}/create-packages-build/",
                     DEFAULT_DOCS_URL))?;
    if ask_default_origin(ui)? {
        ui.br()?;
        ui.para(&format!("Enter the name of your origin. If you plan to publish your packages \
                          publicly, we recommend that you select one that is not already in \
                          use on the {} build service found at {}/.",
                         PRODUCT_NAME, DEFAULT_BUILDER_URL))?;
        ...
        if is_origin_in_cache(&origin, key_cache) {
            ...
            ui.para("You can either create a new signing key now, or, if you are building \
                     ...
                     signing key.")?;
            ui.para(&format!("For more information on the use of origin keys, please consult \
                              the documentation at {}/concepts-keys/#origin-keys",
                             DEFAULT_DOCS_URL))?;
            if ask_create_origin(ui, &origin)? {
                create_origin(ui, &origin, key_cache)?;
            } else {
                ui.para(&format!("You might want to create an origin key later with: `{} \
                                  origin key generate {}'",
                                 CLI_NAME, &origin))?;
            }
        }
    ...
    ui.heading(format!("{} Personal Access Token", PRODUCT_NAME))?;
    ui.para(&format!("While you can perform tasks like building and running {} packages \
                      ...
                      jobs from the {} client will require you to use an access token.",
                     PRODUCT_NAME, PRODUCT_NAME))?;
    ui.para(&format!("The {} Personal Access Token can be generated via the Builder  Profile \
                      page ({}/#/profile). Once you have generated your token, you can enter \
                      it here.",
                     PRODUCT_NAME, DEFAULT_BUILDER_URL))?;
    ui.para(&format!("If you would like to save your token for use by the {} client, please \
                      enter your access token. Otherwise, just enter No.",
                     PRODUCT_NAME))?;
    ui.para(&format!("For more information on using Builder, please read the documentation at \
                      {}/using-builder/",
                     DEFAULT_DOCS_URL))?;
    if ask_default_auth_token(ui)? {
        ui.br()?;
        ui.para(&format!("Enter your {} Personal Access Token.", PRODUCT_NAME))?;
    ...
    if ask_default_ctl_secret(ui)? {
        ui.br()?;
        ui.para(&format!("Enter your {} Supervisor control gateway secret.",
                         PRODUCT_NAME))?;
    ...
    ui.heading(format!("{} Refresh Channel", PRODUCT_NAME))?;
    ui.para(&format!("The {} refresh channel determines which channel to use for core packages \
                      during a build. Common values include 'stable' for stable releases and \
                      'base' for latest supported packages. If you set a refresh channel here, \
                      it will be used as the default when no other channel is specified.",
                     PRODUCT_NAME))?;
    ui.para(&format!("If you would like to save a default refresh channel for use by the {} \
    ...
    if ask_default_refresh_channel(ui)? {
        ui.br()?;
        ui.para(&format!("Enter your default {} refresh channel (e.g., 'stable', 'base').",
                         PRODUCT_NAME))?;
    ...
    #[cfg(windows)]
    {
        let binlink_path =
            Path::new(&*FS_ROOT_PATH).join(Path::new(DEFAULT_BINLINK_DIR).strip_prefix("/")
                                                                         .unwrap());
        ui.heading(format!("{} Binlink Path", PRODUCT_NAME))?;
        ui.para(&format!("The `{}` command-line tool can create binlinks for package binaries \
                          in the 'PATH' when using the 'pkg binlink' or 'pkg install \
                          --binlink' commands. By default, {} will create these binlinks in \
                          the '/hab/bin' directory. This directory will always be included in \
                          your 'PATH' when inside a Studio environment. However, you will want \
                          this directory on your machine's persistent 'PATH' in order to \
                          access binlinks outside of a Studio.",
                         CLI_NAME, PRODUCT_NAME))?;
        if ui.prompt_yes_no("Add binlink directory to PATH?", Some(true))? {
            if !fs::am_i_root() {
                ui.fatal(format!("You must be in an elevated console running as an \
                                  administrator in order to set the machine's PATH. Please \
                                  open a new console running as administrator and run setup \
                                  again to add the {} binlink directory to your path.",
                                 PRODUCT_NAME))?;
            } else {
    ...
    ui.para(&format!("That's all for now. Thanks for using {}!", PRODUCT_NAME))?;
    Ok(())
}
...
fn ask_default_ctl_secret(ui: &mut UI) -> Result<bool> {
    Ok(ui.prompt_yes_no(&format!("Set up a default {} Supervisor control gateway secret?",
                                 PRODUCT_NAME),
                        Some(false))?)
}

fn ask_default_refresh_channel(ui: &mut UI) -> Result<bool> {
    Ok(ui.prompt_yes_no(&format!("Set up a default {} refresh channel?", PRODUCT_NAME),
                        Some(false))?)
}
...
fn prompt_auth_token(ui: &mut UI) -> Result<String> {
    ...
    Ok(ui.prompt_ask(&format!("{} personal access token", PRODUCT_NAME),
                     default.as_deref())?)
}
...
fn prompt_ctl_secret(ui: &mut UI) -> Result<String> {
    ...
    Ok(ui.prompt_ask(&format!("{} Supervisor control gateway secret", PRODUCT_NAME),
                     default.as_deref())?)
}
...
fn prompt_refresh_channel(ui: &mut UI) -> Result<String> {
    ...
    Ok(ui.prompt_ask(&format!("{} refresh channel", PRODUCT_NAME),
                     default.as_deref())?)
}
